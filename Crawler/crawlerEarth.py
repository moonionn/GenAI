...
save_file_name = 'earth'
target = 'future-planet'

root_dir = '../bbc_articles/earth'
...
for i, script in enumerate(scripts):
    ...
    earth_data = find_data(data, target=target)

    if earth_data:
        file_path = os.path.join(root_dir, f'first_article_{save_file_name}.json')

        with open(file_path, 'w', encoding='utf-8') as json_file:
            json.dump(earth_data, json_file, indent=4)

            print(f"JSON data saved to {file_path}")

        extracted_data = []
        for section in earth_data.get('sections', []):
